...
#install.packages("caret", repos = "https://cran.uni-muenster.de/")
#install.packages("CAST", repos = "https://cran.uni-muenster.de/")
#install.packages("blockCV", repos = "https://cran.uni-muenster.de/")
#install.packages("sf", repos = "https://cran.uni-muenster.de/")
#install.packages("terra", repos = "https://cran.uni-muenster.de/")
```
...
#| message: false
#| warning: false
predictor_stack <- terra::rast("data/predictors.tif")
predictor_names <- names(predictor_stack)
spain <- sf::st_read("data/spain.gpkg")

train_points <- sf::st_read("data/temp_train.gpkg")
train_data <- terra::extract(predictor_stack, train_points, bind = TRUE, ID = FALSE) |>
    sf::st_as_sf()
...
test_metrics <- caret::postResample(pred = test_df$prediction, obs = test_df$temp) |>
    round(3) |>
    t() |>
    as.data.frame()
...
#| message: false
#| warning: false
spatial_blocks <- blockCV::cv_spatial(temperature_train, k = 5, progress = FALSE)
...
The output of kNNDM contains a list of row indices of training data points that are used in each CV-iteration (indx_train), as well as of indices that are left out in each iteration (indx_test). These lists can easily be used as input to the `caret::trainControl()` function of caret that defines the CV used in `caret::train()`.
